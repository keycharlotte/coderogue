...
public partial class SkillDatabase : Node
{
    ...
    private Godot.Collections.Dictionary<SkillRarity, List<SkillCard>> _skillsByRarity;
    private Godot.Collections.Dictionary<SkillType, List<SkillCard>> _skillsByType;
    private Godot.Collections.Dictionary<string, List<SkillCard>> _skillsByTag;
    ...
    private void IndexSkills()
    {
        ...
        _skillsByRarity = new Godot.Collections.Dictionary<SkillRarity, List<SkillCard>>();
        _skillsByType = new Godot.Collections.Dictionary<SkillType, List<SkillCard>>();
        _skillsByTag = new Godot.Collections.Dictionary<string, List<SkillCard>>();
        ...
        foreach (var skill in AllSkills)
        {
            ...
            if (!_skillsByRarity.ContainsKey(skill.Rarity))
                _skillsByRarity[skill.Rarity] = new List<SkillCard>();
            _skillsByRarity[skill.Rarity].Add(skill);
            
            // 按类型索引
            if (!_skillsByType.ContainsKey(skill.Type))
                _skillsByType[skill.Type] = new List<SkillCard>();
            ...
            if (skill.Tags != null)
            {
                foreach (var tag in skill.Tags)
                {
                    if (!_skillsByTag.ContainsKey(tag.Name))
                        _skillsByTag[tag.Name] = new List<SkillCard>();
    ...
    public List<SkillCard> GetSkillsByRarity(SkillRarity rarity)
    {
        return _skillsByRarity.GetValueOrDefault(rarity, new List<SkillCard>());
    }
    
    public List<SkillCard> GetSkillsByType(SkillType type)
    {
        return _skillsByType.GetValueOrDefault(type, new List<SkillCard>());
    }
    
    public List<SkillCard> GetSkillsByTag(string tagName)
    {
        return _skillsByTag.GetValueOrDefault(tagName, new List<SkillCard>());
    }
    
    public List<SkillCard> GetAllSkills()
    {
