...
public partial class BuffManager : Node
{
    ...
    {
        _database = GD.Load<BuffDatabase>("res://data/BuffDatabase.tres");
        if (_database == null)
        {
            _database = new BuffDatabase();
            GD.PrintErr("BuffDatabase not found, using empty database");
        }
        _database.Initialize();
        
        _calculator = new BuffCalculator();
    }
    ...
    {
        var effectData = effectInstance.EffectData;
        var value = _calculator.CalculateEffectValue(buff, effectData);
